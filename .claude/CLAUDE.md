...
- ESP-DL 检测器任意一帧返回人脸后立即发布登录成功事件，不额外限制置信度、尺寸或画面位置。
- 检测器启动时若因内存紧张初始化失败，会在人脸模式下每 2 秒自动重试，避免本次开机永久无法使用人脸登录。
- 摄像头取帧由互斥锁串行化，流状态使用原子变量；RGB565 帧会校验缓冲索引、有效长度和行跨度，不完整帧最多重试三次，缓冲归还失败立即终止本次采集。
- 该模式用于演示，不提供身份认证或活体检测能力。

## 摄像头视觉工具

- `self.camera.take_photo` 由一个常驻、串行的低优先级工作任务执行，完成后通过 `Application::Schedule()` 回主任务发送 MCP 回复，避免拍照、JPEG 编码和网络上传阻塞应用事件循环；队列满时直接返回忙碌，不会并发截图或串帧。
- ESP32-P4 开启外部任务栈时，相机工具的 12 KB 栈分配在 PSRAM，避免挤占片内 SRAM。
- P4 截图直接编码为 PSRAM 中的 JPEG 后上传，不再使用额外编码线程和 JPEG 队列；编码期间短暂持有摄像头互斥锁，网络上传期间不占用摄像头锁。
- JPEG 通过 ESP-Hosted Wi-Fi 时按 1 KB 分块并间隔 10 ms 上传，限制同时占用的 1536 字节片内 DMA 传输块数量，降低连续截图时内存耗尽风险。
- 视觉 HTTP 请求超时为 15 秒，每段写入均检查结果；连接、编码或上传失败会返回 MCP 错误，不会永久等待。
