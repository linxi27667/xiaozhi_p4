...
void LcdDisplay::InitializeLcdThemes() {
    ...
    light_theme->set_background_color(lv_color_hex(0xEEF3F7));
    light_theme->set_text_color(lv_color_hex(0x0F172A));
    light_theme->set_chat_background_color(lv_color_hex(0xE8EEF5));
    light_theme->set_user_bubble_color(lv_color_hex(0xBBF7D0));
    light_theme->set_assistant_bubble_color(lv_color_hex(0xF8FAFC));
    light_theme->set_system_bubble_color(lv_color_hex(0xE2E8F0));
    light_theme->set_system_text_color(lv_color_hex(0x334155));
...
void LcdDisplay::OnShellNavClicked(lv_event_t* e) {
    ...
    if (target == display->shell_chat_btn_) {
        page = ShellPage::Chat;
    } else if (target == display->shell_overview_btn_) {
...
void LcdDisplay::SwitchShellPage(ShellPage page) {
    if (!smart_home_page_) {
        return;
    }

    const char* page_name = "chat";
    switch (page) {
    ...
    ESP_LOGI(TAG, "Shell page switch: %s", page_name);

    const bool smart_home = page != ShellPage::Chat;
    lv_obj_t* chat_objects[] = {
        container_,
        emoji_box_,
        preview_image_,
        top_bar_,
    ...
    for (auto* obj : chat_objects) {
        if (!obj) {
            continue;
        }
        if (smart_home) {
            lv_obj_add_flag(obj, LV_OBJ_FLAG_HIDDEN);
        } else if (obj != bottom_bar_ && obj != low_battery_popup_ && obj != preview_image_) {
...
void LcdDisplay::CreateSmartHomeShell() {
    ...
    if (container_) {
        lv_obj_set_pos(container_, kShellSidebarWidth, 0);
        lv_obj_set_size(container_, content_width, LV_VER_RES);
        lv_obj_set_style_bg_color(container_, lv_color_hex(0xEEF3F7), 0);
    }
    if (top_bar_) {
        lv_obj_set_pos(top_bar_, kShellSidebarWidth, 0);
        lv_obj_set_width(top_bar_, content_width);
    }
    if (status_bar_) {
        lv_obj_set_pos(status_bar_, kShellSidebarWidth, 0);
        lv_obj_set_width(status_bar_, content_width);
    }
    if (notification_label_) {
        lv_obj_set_width(notification_label_, content_width * 8 / 10);
    }
    if (status_label_) {
        lv_obj_set_width(status_label_, content_width * 8 / 10);
    }
    if (emoji_box_) {
        lv_obj_align(emoji_box_, LV_ALIGN_CENTER, kShellSidebarWidth / 2, 0);
    }
    if (preview_image_) {
    ...
    lv_obj_set_pos(smart_home_page_, kShellSidebarWidth, 0);
    lv_obj_set_size(smart_home_page_, content_width, LV_VER_RES);
    lv_obj_set_style_bg_color(smart_home_page_, lv_color_hex(0xF5F7FB), 0);
    lv_obj_set_style_bg_opa(smart_home_page_, LV_OPA_COVER, 0);
    lv_obj_add_flag(smart_home_page_, LV_OBJ_FLAG_HIDDEN);
