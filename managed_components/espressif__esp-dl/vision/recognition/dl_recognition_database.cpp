...
esp_err_t DataBase::load_database_from_storage(int feat_len)
{
    ...
    if (feat_len != m_meta.feat_len) {
        const int stored_feat_len = m_meta.feat_len;
        fclose(f);

        // A face model update can change the feature-vector length. Keeping the
        // old header makes every later enrollment fail forever, so preserve the
        // incompatible database and start a new one for the active model.
        const std::string backup_path = m_db_path + ".incompatible";
        remove(backup_path.c_str());
        if (rename(m_db_path.c_str(), backup_path.c_str()) != 0) {
            ESP_LOGE(TAG, "DB_FIX_V1 failed to back up incompatible database: stored=%d expected=%d",
                     stored_feat_len, feat_len);
            return ESP_FAIL;
        }

        ESP_LOGW(TAG, "DB_FIX_V1 backed up incompatible database: stored=%d expected=%d path=%s",
                 stored_feat_len, feat_len, backup_path.c_str());
        return create_empty_database_in_storage(feat_len);
    }
    uint16_t id;
